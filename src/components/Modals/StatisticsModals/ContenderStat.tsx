      ...
      <div className="absolute top-0 bottom-0 left-0 right-0 z-50 bg-black/50
        ...
        <div className="relative min-w-[700px] max-h-[90%] overflow-auto flex justify-center items-center flex-col p-4 bg-white rounded-[10px]">
          ...
          <h1 className="w-full flex items-center flex-row font-poppins font-bold pt-2">
            <IoStatsChart className="w-[20px] h-[20px] mr-3" />
            Category Statistics
          </h1>
          ...
          <div className="w-full flex justify-start items-start flex-row space-x-2 text-[12px]">
            <p onClick={() => setSelectedChart(1)} className={`${selectedChart === 1 ? 'bg-[#0C35BC] font-bold text-white' : 'bg-[#0C35BC]/80'} font-poppins mt-4 py-1 px-4 rounded-[4px]  select-none  cursor-pointer transition-all duration-100`}>
              Bar Chart
            </p>

            <p onClick={() => setSelectedChart(2)} className={`${selectedChart === 2 ? 'bg-[#0C35BC] font-bold text-white' : 'bg-[#0C35BC]/80'} font-poppins mt-4 py-1 px-4 rounded-[4px] select-none  cursor-pointer transition-all duration-100`}>
              Line Chart
            </p>

            <p onClick={() => setSelectedChart(3)} className={`${selectedChart === 3 ? 'bg-[#0C35BC] font-bold text-white' : 'bg-[#0C35BC]/80'} font-poppins mt-4 py-1 px-4 rounded-[4px] select-none  cursor-pointer transition-all duration-100`}>
              Race Chart
            </p>

            <p onClick={() => setSelectedChart(4)} className={`${selectedChart === 4 ? 'bg-[#0C35BC] font-bold text-white' : 'bg-[#0C35BC]/80'} font-poppins mt-4 py-1 px-4 rounded-[4px]  select-none cursor-pointer transition-all duration-100`}>
              Pie Chart
            </p>

            <p onClick={() => setSelectedChart(5)} className={`${selectedChart === 5 ? 'bg-[#0C35BC] font-bold text-white' : 'bg-[#0C35BC]/80'} font-poppins mt-4 py-1 px-4 rounded-[4px]  select-none cursor-pointer transition-all duration-100`}>
              Raw Table
            </p>
          </div>
